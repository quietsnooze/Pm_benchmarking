...
| Build the annual provision-coverage panel `processed_inputs/firm_provisions_annual.csv` from EBA transparency-exercise CSVs (skips cleanly when the raw files are absent) | `uv run extract-provisions` |
| All of the above, in order — equivalent to a "build" of `processed_inputs/` from scratch | `uv run ingest` |
